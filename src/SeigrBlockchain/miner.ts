...
class Miner {
  ...
  constructor({ chain, transactionPool, wallet, pubsub }: any) {
    this.chain = chain;
  ...
  static walletMiner({ chain, transactionPool, wallet }: any) {
  ...
  static rewardMiner({ chain, transactionPool, wallet }: any) {
    const minerWalletOutput = {
      address: wallet.publicKey,
      amount: config.miningReward,
    };
  ...
  static validTransactionData({ chain }: any) {
  ...
  static validStakeData({ chain }: any) {
    for (let i = 1; i < chain.length; i++) {
      const block = chain[i];
      const transactionSet = new Set();
      let rewardTransactionCount = 0;
  ...
  static validChain(chain: Block[]) {
    ...
    for (let i = 1; i < chain.length; i++) {
      ...
      const validatedHash = Block.hash(
        timestamp,
        lastHash,
        data,
        nonce,
  ...
  static adjustStakeDifficulty({ originalBlock, timestamp }: any) {
    const { difficulty } = originalBlock;
  ...
  static stakeBlock({ lastBlock, data }: any) {
    let hash, timestamp;
    const lastHash = lastBlock.hash;
    let { difficulty } = lastBlock;
    let nonce = 0;
    ...
    do {
      nonce++;
      timestamp = Date.now();
      difficulty = Block.adjustStakeDifficulty({
        originalBlock: lastBlock,
        timestamp,
      });
      hash = Block.hash(timestamp, lastHash, data, nonce, difficulty);
  ...
  static validateBlock(block: Block) {
    const {
      timestamp,
      lastHash,
      hash,
  ...
  static validateStakeBlock(block: Block) {
    const {
      timestamp,
      lastHash,
      hash,
